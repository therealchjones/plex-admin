...
async function throwFatalError (message = '', error = null) {
  if (debugMode) { console.debug(`Throwing fatal error. Message: ${message}. Error: ${error}.`) }
  if (message.length === 0) { message = 'An unrecoverable error occurred. Halting.' }
...
async function showErrorDialog (message = '') {
  ...
  if (message.length === 0) { message = 'An error occurred. Console output may include more information.' }
...
async function buildErrorDialog () {
  ...
  modal.innerHTML = '<div class="modal-dialog modal-dialog-centered" role="document"> <div class="modal-content"> <div class="modal-body"></div> </div> </div>'
...
async function buildShowEntry (showData, template) {
  ...
  const itemId = 'show-item-' + showData.titleSlug
  const sonarrId = showData.id
...
async function loadShows () {
  ...
  if (!showsDb.json || showsDb.json.response === undefined) { throw new Error('No list of shows in database') }
  const list = document.getElementById('shows-list')
  const template = document.getElementById('show-item-aps-template')
  const rowPromises = []
...
async function loadMovies () {
  return Promise.resolve(getDb('movies'))
    .then((moviesDb) => {
      ...
      if (!moviesDb.json || moviesDb.json.response === undefined) { throw new Error('No list of movies in database') }
    ...
    .then((list) => {
      const loading = document.getElementById('loading-movies')
...
async function hasAuth () {
  ...
  const response = await requestData('sonarr', '/ping')
  if (!response.ok) {
    if (debugMode) {
